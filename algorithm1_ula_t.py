...
class TruckULASolver:
    """Algorithm 1: Unidirectional Label-Setting Algorithm for Truck (ULA-T)"""
    ...
    def solve(self, duals: Dict[str, float]) -> List[Column]:
        """Solve truck routing subproblem using ULA-T"""
        columns = []
            ...
            for current_node in range(len(self.nodes)):
                ...
                for label in current_labels:
                    ...
                    for next_node in range(len(self.nodes)):
                        ...
                        # Add resource cost based on time slices used
                        ...
                        new_label = label.extend_to(
                            next_node, travel_time, travel_cost + reduced_cost, -pickup_load
                        )
            ...
            for label in label_sets[node_id].labels:
                ...
                if 0 not in label.path[1:]:  # Haven't returned to depot yet
                    ...
                    if total_reduced_cost < -1e-6:
                        ...
                        route_details = self._calculate_route_details(final_path, label)
                        
                        column = Column(
                            type='truck',
                            id=f"truck_route_{len(columns)}",
    ...
    def _calculate_route_details(self, path: List[int], final_label: TruckLabel) -> Dict[str, Any]:
